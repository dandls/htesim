...
Settings of \cite{Nie_Wager_2020}, were also employed by \cite{Dandl_Hothorn_Seibold_2022} to compare multiple forest-based methods.
A dedicated dataset with the study settings and method performances is part of this package (\code{data("dandl")})
...
replicate the study framework of \cite{Dandl_Hothorn_Seibold_2022}.
