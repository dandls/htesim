...
a Weibull model and a fournomial model for the outcome are supported.
...
These variables are labeled as \code{X1}, \code{X2}, and so on.
...
With the \proglang{R} package \pkg{lattice} we compare the performance of the forest-based methods and the linear model visually.
...
Each method will have an own color coding.
<<color>>=
...
This color coding is needed for further plotting options.
...
\newpage
<<plot, results = "asis", fig.height = 14.5, fig.width = 14>>=
plot_results(subset1, sc = sc)
@
\newpage
<<plot2, results = "asis", fig.height = 14.5, fig.width = 14>>=
plot_results(subset2, sc = sc)
@
\newpage
<<plot3, results = "asis", fig.height = 14.5, fig.width = 14>>=
plot_results(subset3, sc = sc)
@
\newpage
<<plot4, results = "asis", fig.height = 14.5, fig.width = 14>>=
