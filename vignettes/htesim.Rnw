...
structural equation models and not parametric distribution functions like \pkg{htesim} (\pkg{simcausal} package \citep{Sofrygin_Laan_Neugebauer_2017}, \pkg{lavaan} \citep{Rosseel_2012} and \pkg{simsem} \citep{Pornprasertmanit_Miller_Schoemann_2020}).
Other packages only cover special cases like generalized multistate models or survival models
(\pkg{survsim} \citep{Morina_Navarro_2014}, \pkg{gems} \citep{Blaser_Salazar_Estill_2015}).
...
In the following, we give details on these functions accompanied by an
illustrative example that replicates the first experiment of \cite{Wager_Athey_2018}.
...
If their are normally distributed, the user could optionally pass only a
function for the standard deviation (\code{sd}).
Furthermore, a proportion of the treatment effect could be added to the prognostic effect (\code{pi}).

As an example we have a look on the first data generating process of the simulation
...
All propensity score , prognostic effect and treatment effect functions of \cite{Wager_Athey_2018}, \cite{Athey_Tibshirani_Wager_2019} and \cite{Nie_Wager_2020} are already implemented in \pkg{htesim}.
...
For our example, we use \code{pF_x1} for the
propensity score function and \code{mF_x1} for the prognostic effect function.

The following code generates the manual to replicate the study setting.
The returned object is a list of class \code{dgp}.
<<ex:dgp>>=
dgp1 <- dgp(p = pF_x1, m = mF_x1, t = 0, pi = 0, model = "normal",
  xmodel = "unif")
dgp1
class(dgp1)
@
...
The study dataset could be exactly reproduced in another run by specifying a
\code{seed}.
...
a normally distributed outcome \code{y} was sampled and the treatment indicator
...
head(testdf)
@
The test dataset consist of covariates (the number is equal to \code{dim})
...
To evaluate if a method accurately estimates the treatment effect or potentially (but less common) the
prognostic effect, propensity scores or standard deviation we need the
ground true effects or scores for the test dataset.
For this purpose the S3 method \code{predict.dgp} exists. It needs as an input
the \code{dgp object} generated by \code{dgp} and the new data frame.
...
\pkg{htesim} also offers the flexibility for users to specify and simulate from their own functions
and provides the necessary functionality to replicate previous studies.
This is demonstrated in this chapter.
For the former we use a self-chosen example and for the latter we replicate the
first study setting of \cite{Dandl_Hothorn_Seibold_2021} which is also
included in \pkg{htesim} (\code{data("studyres")}).
...
If we want to simulate 500 observations from this dgp we could simply call
<<simowntE>>=
...
Columns \code{p}, \code{m}, \code{t}, \code{sd}, \code{pi} \code{pi}, \code{model} and \code{xmodel}
...
The information in this dataset is enough to reproduce the data generating process.
...
<<getdatar>>=
dgp1 <- do.call(dgp,
  as.list(study1)[c("p", "m", "t", "sd", "pi", "model", "xmodel")])
# equivalent to
dgp1 <- dgp(p = study1$p, m = study1$m, t = study1$t, sd = study1$sd,
  pi = study1$pi, model = study1$model, xmodel = study1$xmodel)
...
The results for each method are given in columns 14 to 23 in \code{studyres}.
...
<<reshape>>=
res <- reshape(studyres, direction = "long", varying = methodnams, idvar = c("p",
  "m", "t", "sd", "pi", "model", "xmodel", "nsim", "dim", "nsimtest", "seed"),
...
@
Each row corresponds to the results of \textit{one} method for \textit{one} study setting
(unique data generating process, number of training observations and dimensions)
and \textit{one} replication.
Each row of our plot should correspond to one data generating process (i.e., \code{setup}) and
...
refactor <- function(df, method.nams) {
  ...
  df <- df[, c("algorithm", "setup", "repl", "nd", "pi", "seed", "value")]
...
Next we set up plotting functions of the \proglang{R} package \pkg{lattice}
...
\begin{itemize}
\item with overlay meaning \code{pi} is equal to 0: setups 1 to 8
<<set1>>=
subset1 <- subset(res, pi == 0 & setup %in% 1:16)
@
\item without overlay meaning \code{pi} is equal to 0.5: setups 9 to 16
<<set2>>=
subset2 <- subset(res, setup %in% 1:16 & pi == 0.5)
@
\end{itemize}
\item Settings introduced by \cite{Nie_Wager_2020}
\begin{itemize}
\item with overlay meaning \code{pi} is equal to 0: setups 17 to 20
<<set3>>=
subset3 <- subset(res, setup %in% 17:24 & pi == 0)
@
\item without overlay meaning \code{pi} is equal to 0.5: setups 21 to 24
<<set4>>=
subset4 <- subset(res, setup %in% 17:24 & pi == 0.5)
@
\end{itemize}
\end{itemize}
A \code{pi} of 0.5 means that half of the treatment effect is added to the
prognostic effect.
Finally we can run the plot the results
\newpage
<<plot, fig.width=10, fig.height = 10.5, fig.width = 11>>=
plot_results(subset1, sc = sc)
@
\newpage
<<plot2, fig.width=10, fig.height = 10.5, fig.width = 11>>=
plot_results(subset2, sc = sc)
@
\newpage
<<plot3, fig.width=10, fig.height = 10.5, fig.width = 11>>=
plot_results(subset3, sc = sc)
@
\newpage
<<plot4, fig.width=10, fig.height = 10.5, fig.width = 11>>=
plot_results(subset4, sc = sc)
@

If users add another method, they only need to adjust the vector of method names
\code{methodnams} and their color coding \code{cols}.
All other chunks of this section should work without any adjustments.

\section{Summary} \label{summary}
This vignette introduced the package \pkg{htesim} that
provides an extensive simulation study framework to evaluate methods to estimate
heterogeneous treatment effects.
It gave some illustration on how to put predefined functions for
prognostic and predictive effects and propensity scores to generate
a simulation data sets to put methods for treatment effects to the proof.
Furthermore, it explained how users can integrate their own functions which hopefully
removes hurdles to use this package for their prospective simulation studies.
The vignette also provided the necessary functions to reproduce the
study framework of \cite{Dandl_Hothorn_Seibold_2021} and to plot their results for
forest-based methods.
This should motivate users to compare their methods against it.
