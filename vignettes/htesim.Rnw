...
range from deploying classical lasso regression (TODO), over recursive partitioning \citep{Athey_Tibshirani_Wager_2019, Seibold_Zeileis_Hothorn_2017} to
...
illustrative example which replicates the first experiment of \cite{Wager_Athey_2018}.
...
with $(W | \rX = \rx) \sim B(1, \pi(\rx))$.
Covariates $\rX$ are sampled from a uniform distribution
$(\rX \sim U([0, 1]^P))$ with $P = \{2, 5, 10, 15, 20, 30\}$.

All propensity score , prognostic effect and treatment effect functions of \cite{Wager_Athey_2018}, \cite{Athey_Wager_2019} and \cite{Nie_Wager_2020} are already implemented in \pkg{htesim}.
Please consult the documentation of \code{pE}, \code{mE} and \code{tE}
for an overview and in-depth definition of these functions.

With the following code we generate the necessary manual to
replicate the study setting which is essentially a list of class \code{dgp}.
<<ex:dgp>>=
...
For our example we use $P = 2$.
Training and test data had $n = 500$ observations each.
...
\cite{Dandl_Hothorn_Seibold_2021}, the study setting and the corresponding results of the forest approaches are
...
\code{seed} helps us to receive the exact and not approximate study data set of \cite{Dandl_Hothorn_Seibold_2021}.
...
% @
%
% \cite{Dandl_Hothorn_Seibold_2021} were only interested in the treatment effect estimation.
