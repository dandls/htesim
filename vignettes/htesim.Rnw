...
A dedicated dataset of the study settings and performances of these methods  is part of this package (\code{data("studyres")})
...
Furthermore, a proportion \code{pi} $\in \mathbb{R}$ of the treatment effect could be added to the prognostic effect.
...
The results for each method are also part of the given data frame \code{studyres}.
...
We can apply the linear interaction model to each of 9600 simulation settings specified in \code{studyres}.
The following helper functions create a study dataset from one study setting,
fits the interaction model on it, estimates the treatment effect and
returns the MSE.
The results are attached to \code{studyres}.
...
run_linmod <- function(i) {
  ...
  # define dgp
  dgp1 <- do.call(dgp,
  as.list(row)[c("p", "m", "t", "sd", "pi", "model", "xmodel")])
...
mse <- sapply(1:nrow(studyres), FUN = run_linmod)

# add results to studyres
studyres$linmod <- mse
...
mykey <- list(space="top", rectangles=list(col=cols),
  text=list(names(cols)), columns = ceiling(length(methodnams)/2))
...
\begin{itemize}
\item without overlay meaning \code{pi} is equal to 0: setups 1 to 8
<<set1>>=
subset1 <- subset(res, pi == 0 & setup %in% 1:16)
@
\item with overlay meaning \code{pi} is equal to 0.5: setups 9 to 16
<<set2>>=
...
\begin{itemize}
\item without overlay meaning \code{pi} is equal to 0: setups 17 to 20
<<set3>>=
subset3 <- subset(res, setup %in% 17:24 & pi == 0)
@
\item with overlay meaning \code{pi} is equal to 0.5: setups 21 to 24
<<set4>>=
subset4 <- subset(res, setup %in% 17:24 & pi == 0.5)
@
\end{itemize}
\end{itemize}
If \code{pi} is 0.5, half of the treatment effect is added to the
prognostic effect.
...
If users want to visualize the result of another method, they need to attach the MSEs of their method to \code{studyres} and need to adjust the vector of method names \code{methodnams} and their color coding \code{cols}.
